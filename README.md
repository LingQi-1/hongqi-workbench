...
- **新闻**：免费 RSS 综合频道 + 红旗专属频道，分类 / 收藏 / 刷新。
...
## 自定义
- **油价实时接口**：在 `js/price.js` 的 `SEED_PRICES` 处接入实时数据源，或替换 `openPriceEditor` 的保存逻辑。
- **红旗新闻 RSS**：在 `js/news.js` 的 `RSS_FEEDS` 增加红旗官方 RSS 地址即可（当前为示例内容）。
